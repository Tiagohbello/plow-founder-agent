...
never sent automatically. Each check refreshes the mapped `next_step` column so
the sheet carries the current recommendation; there is no automatic invitation,
hold deletion or factual CSV write. There are no repeated reminders for unchanged pending suggestions. A
