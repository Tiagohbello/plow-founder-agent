...
private HOLDs on the active configured default calendar and, on the next revisit
of that contact, removes obsolete HOLDs absent from the current hold_plan — never
ask for a go-ahead to place or clear them. It never sends a third-party message or
creates a real invitation, even when `calendar_manage` is autonomous. Unattended
checks write only the `next_step` of a contact's page in the root this agent owns
(advice the founder can ignore, not a claim about the world) and, after verified
private HOLD placement or deletion, reconcile the page's factual `holds` to match
the calendar. `status` and `proposed` still move only after verified execution of
an approved suggestion. Private HOLDs require a linked draft, target the active
configured default calendar, and use `hold_plan` and `operations.py`
`create_private_hold` / `delete_private_hold` without founder approval;
invitations in `calendar_plan` still need it.
